            ...
            if (DoIt && ((layer.value | (1<<other.gameObject.layer)) == layer.value))
            {
                ...
                if (block != null && block.tank != null && !block.tank.beam.IsActive)
                {
                    ...
                    if (force.magnitude < threshold)
                    {
                        ...
                        block.tank.rbody.AddForceAtPosition((Vector3.ProjectOnPlane(-rbody.GetPointVelocity(pos), block.transform.rotation * Vector3.up) + force) * strength * 7 + block.transform.rotation * Vector3.down * threshold * 5, pos, ForceMode.Impulse);
                    }
